...
var Form = View.extend(function(action, method) {
	...
	this.view.on("submit", function(e) {
		...
		$(this).find("input, textarea, select").each(function(i) {
			...
			// trigger a change event before submission
			$(this).trigger('change');
		});
...
var Input = View.extend(function(type, name, value) {
	...
	this.valueListener = this.listenTo("value", function(val, self) {
		console.log("triggering listen", input.name, val);
		self.view.val(val);
	});

	// automatically update values on change
	this.view.on("change", function(e) {
		console.log("updating value", input.name, input.value, $(this).val());
