...
var View = Model.partial({}).extend(function(html) {
	...
	// trap attempts to change view to make sure that the change gets updated in the DOM
	this.setTrap("view", function(newView, properties) {
	...
	// trap attempts to change the views to make them a replace operation instead
	this.setTrap("views", function(newViews) {

		this.views.replace(newViews);
	});
...
View.prototype.defineAttr = function(name, val) {
	...
	this.listenTo(name, function(new_val) {
		...
		this.view.setProp(name, new_val);
	});
...
DOMBinding.DOMReady(function() {
	...
	View.Body.__properties.view = new DOMBinding(document.body);
